...
int mMenu(string playerNames[2])
{
	...
	while (1)
	{
		...
		case ENTER:
			...
			if (altList==false && pos[0])
			{
				alternateList(pos, 1);
				altList = true;
			}

			if (altList == false && pos[1])
			{
				//showrules();
			}
...
void banner()
{
	...
	cout << " _______  _   ______	  _______	______      ______	  __	  __	  _	" << endl;
	cout << "|   ____|| | |	  __  \	 |  _____| |   __  \   /   _  \	 |	|	 | 	|	 | |" << endl;
	cout << "|  |____ | | |	 |__)  | |	|____  |  |__)  | |   / \  | |	|	 |	|	 | |" << endl;
	cout << "|   ____|| | |		   / |	 ____| |       /  |	 |___| | |	|	 |	|	 | |" << endl;
	cout << "|  |	  | | |	  _   /	 |	|	   |   __  \  |	  __   | |	|	 |	|	 |_|" << endl;
	cout << "|  |	  | | |	 | \  \	 |  |____  |  |__)  | |	 |	|  | |	|___ |	|___  _	" << endl;
	cout << "|__|	  |_| |__|	\__\ |_______| |_______/  |__|	|__| |______||______||_|" << endl;
	cout << "																			" << endl;
}
#endif